...
En caso de usar Visual Studio Code, tener las herramientas de compilación `StrawBerry Perl` y administración de paquetes `Miktek Console`. En `settings.json` a nivel de usuario desde cualquier parte, respetando la sintaxis de JSON que no afecte otras extensiones evitando el punto anterior para la compilación manual.
